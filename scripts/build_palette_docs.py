...
def parse_conf(path):
    ...
    for line in text.splitlines():
        m = re.match(r"^(cursor|foreground|background|selection_foreground|selection_background|"
                      r"color\d{1,2}|active_tab_foreground|active_tab_background|"
                      r"inactive_tab_foreground|inactive_tab_background)\s+(#[0-9a-fA-F]{6})", line)
        if m:
...
THEMES = {
    "shibui": {
        ...
        "accents": [
            ("color1", "color9", "Beni", "seal red", "The calligraphy scroll's cinnabar stamp — quiet warmth, not alarm."),
            ("color2", "color10", "Cha", "tea-sage", "The tatami weave — dry sage, neutral and grounding."),
            ("color3", "color11", "Kin", "gilt", "The scroll's gold-leaf border, aged and quieted."),
            ("color4", "color12", "Ai", "indigo", "The kimono's cool cloth — the hero hue of this palette."),
            ("color5", "color13", "Fuji", "wisteria", "Mixed from Beni and Ai, sitting between scroll and cloth."),
            ("color6", "color14", "Mizu", "water", "Cha and Ai mixed — tatami green meets kimono blue."),
        ],
    },
    "obi": {
        "label": "obi",
        ...
        "accents": [
            ("color1", "color9", "Beni", "seal red", "The calligraphy scroll's cinnabar stamp."),
            ("color2", "color10", "Cha", "tea-sage", "The tatami weave — dry sage, unchanged."),
            ("color3", "color11", "Kaki", "persimmon", "Ferra's warm coral-salmon — the yellow slot reads as ripe fruit, not gilt."),
            ("color4", "color12", "Ai", "indigo, whispered", "Demoted from hero to the quietest accent — lighter and less saturated."),
            ("color5", "color13", "Sakura", "cherry blossom", "Ferra's pink opened up — deeper plum in shibui, blossom here."),
...
def nav(current):
    ...
    for i, (href, label) in enumerate(links):
        cls = ' class="current"' if (current == "index" and href == "index.html") or href == f"{current}.html" else ""
...
def ground_grid(colors):
    rows = [
        ...
        ("Raised", "panel / tab", colors.get("active_tab_background", colors["background"])),
...
def build_index_page():
    ...
    for key in ORDER:
        ...
        swatch_keys = ["background", "color1", "color2", "color3", "color4", "color5", "color6", "color7"]
        row = "".join(f'<span style="background:{tcolors[k]}"></span>' for k in swatch_keys)
    ...
    compare_rows = []
    for label, keyname in [("background", "background"), ("foreground", "foreground"),
                            ("Ai &middot; blue", "color4"), ("Cha &middot; green", "color2")]:
        cells = []
        for key in ORDER:
            tcolors = parse_conf(THEMES_DIR / f"{key}.conf")
            hexv = tcolors.get(keyname, "—")
            cells.append(f'<td><span class="chip-inline" style="background:{hexv}"></span>{hexv}</td>')
...
"""
    return page_shell("kasane &middot; shibui / obi", "Two muted kitty terminal themes for calm, accessible reading.", root_vars(colors), body, "index")
